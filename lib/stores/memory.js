...
var memoryStore = function(args) {
    ...
    self.ttl = function(key, cb) {
        const remainingTTLMS = lruCache.getRemainingTTL(key);
        /**
         * we shouldn't have a negative TTL, lruCache is in MS so convert to seconds, then round down
         * rounding down may be an incorrect assumption
         */
        const remainingTTLSeconds = Math.max(0, Math.floor(remainingTTLMS/1000))
        cb(null, remainingTTLSeconds);
    };
